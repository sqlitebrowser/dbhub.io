...
func AnalysisRecordUserStorage(userName string, recordDate time.Time, spaceUsedStandard, spaceUsedLive int64, numStandardDB, numLiveDB int) (err error) {
	dbQuery := `
		WITH u AS (
			SELECT user_id
			FROM users
		...
		INSERT INTO analysis_space_used (user_id, analysis_date, standard_databases_bytes, live_databases_bytes, standard_databases_qty, live_databases_qty)
		VALUES ((SELECT user_id FROM u), $2, $3, $4, $5, $6)`
	commandTag, err := pdb.Exec(context.Background(), dbQuery, userName, recordDate, spaceUsedStandard, spaceUsedLive, numStandardDB, numLiveDB)
	if err != nil {
		log.Printf("Adding record of storage space used and database qty by '%s' failed: %s", userName, err)
		return
	}
	if numRows := commandTag.RowsAffected(); numRows != 1 {
		log.Printf("Wrong number of rows (%d) affected when recording the storage space used and database qty by '%s'", numRows, userName)
	}
	return
}
